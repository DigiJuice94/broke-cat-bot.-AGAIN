...
async function main(){
  const wallet=new WalletService(),birdeye=new Birdeye(),helius=new Helius(),jupiter=new Jupiter(wallet),trader=new Trader(wallet,birdeye,jupiter);
  const scanner=new Scanner(birdeye,helius,jupiter,c=>trader.buy(c));
  log.info("🐱 BROKE CAT BOT v1.2 — DATA EFFICIENCY");
  ...
  log.info(`Data stack: DEX Screener batch + Birdeye queued/cache + Helius queued/selective + Jupiter finalists`);
  log.info(`Birdeye pacing: ${config.birdeyeMinIntervalMs}ms | deep top ${config.birdeyeDeepCandidates} | Helius pacing: ${config.heliusMinIntervalMs}ms | deep top ${config.heliusDeepCandidates}`);
  ...
  if(!config.heliusApiKey)log.warn("HELIUS_API_KEY missing — on-chain intelligence reduced.");
