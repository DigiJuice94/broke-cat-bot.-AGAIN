...
export const config = {
  ...
  flameAutoBuyScore: num("FLAME_AUTO_BUY_SCORE", 90),
  flameMinDataConfidence: num("FLAME_MIN_DATA_CONFIDENCE", 70),
  ...
  minPositionUsd: num("MIN_POSITION_USD", 5),
  basePositionUsd: num("BASE_POSITION_USD", 10),
  maxPositionUsd: num("MAX_POSITION_USD", 15),
  tradeCooldownMs: num("TRADE_COOLDOWN_MS", 60_000),
  flameBypassCooldown: bool("FLAME_BYPASS_COOLDOWN", true),
  feeAwareEntryEnabled: bool("FEE_AWARE_ENTRY_ENABLED", true),
  minRouteQualityPct: num("MIN_ROUTE_QUALITY_PCT", 88),
  minExpectedNetEdgePct: num("MIN_EXPECTED_NET_EDGE_PCT", 12),
  routeCostMultiplier: num("ROUTE_COST_MULTIPLIER", 1.5),
  estimatedNetworkFeeSol: num("ESTIMATED_NETWORK_FEE_SOL", 0.00002),
  ...
  hardStopLossPct: num("HARD_STOP_LOSS_PCT", 10),
  earlyFailureLossPct: num("EARLY_FAILURE_LOSS_PCT", 5),
  earlyFailureWindowSec: num("EARLY_FAILURE_WINDOW_SEC", 90),
  runnerFirstTakeProfitPct: num("RUNNER_FIRST_TAKE_PROFIT_PCT", 40),
  runnerFirstSellPct: num("RUNNER_FIRST_SELL_PCT", 30),
  runnerSecondTakeProfitPct: num("RUNNER_SECOND_TAKE_PROFIT_PCT", 75),
  runnerSecondSellOriginalPct: num("RUNNER_SECOND_SELL_ORIGINAL_PCT", 50),
  moonBagTakeProfitPct: num("MOON_BAG_TAKE_PROFIT_PCT", 200),
  moonBagTrailingStopPct: num("MOON_BAG_TRAILING_STOP_PCT", 20),
  positionStateFile: process.env.POSITION_STATE_FILE ?? "broke-cat-live-state.json",
  walletReconciliationEnabled: bool("WALLET_RECONCILIATION_ENABLED", true),
  recoverUnknownWalletTokens: bool("RECOVER_UNKNOWN_WALLET_TOKENS", true),
