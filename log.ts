...
export const log={
  ...
  scan(data:{name:string;symbol:string;priceUsd?:number;score:number;confidence:number;status:string;reason?:string;sources?:string[];rankText?:string;details?:{
    ...
    top10HolderPct?:number;deep?:string}}){
    ...
    const detail=d?` | B/S(${flowWindow}):${buys??"?"}/${sells??"?"} Vol:${vol==null?"?":`$${Math.round(vol)}`} Liq:${d.liquidityUsd==null?"?":`$${Math.round(d.liquidityUsd)}`} Holders:${d.holderCount??"?"} Top10:${d.top10HolderPct==null?"?":`${d.top10HolderPct.toFixed(1)}%`}${d.deep?` Deep:${d.deep}`:""}`:"";
